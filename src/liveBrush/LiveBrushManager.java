...
public class LiveBrushManager
{
	...
	//TODO add source folder path if needed
	
	public LiveBrushManager(PApplet applet, String absoluteSourceFolderPath)
	{
		this.applet = applet;
		this.absoluteSourceFolderPath = getCheckedAbsoluteSourceFolderPath(absoluteSourceFolderPath);
	...
	{
		this.applet = applet;
		this.absoluteSourceFolderPath = getCheckedAbsoluteSourceFolderPath(applet.sketchPath() + "/livebrushes/");
		liveBrushes = getLiveBrushesFromExternalSource();
		liveBrushInstances = new ArrayList<LiveBrushInstance>();
		setFileChangeListener(absoluteSourceFolderPath, 1000);
	}
	...
	private void openLiveBrushFolder()
	{
		try
		{
			java.awt.Desktop.getDesktop().open(new java.io.File(getCheckedAbsoluteSourceFolderPath(this.absoluteSourceFolderPath)));
		}
	...
	private void createLiveBrushDocument(String name)
	{
		try{
			...
			String filePath = getCheckedAbsoluteSourceFolderPath(absoluteSourceFolderPath) + "/" + name + ".java";
	...
	{
		String absoluteSourceFolderPath = getCheckedAbsoluteSourceFolderPath(this.absoluteSourceFolderPath);
	...
	/**
	 * Checks the folder path for existence
	 * @param absoluteSourceFolderPath The path to the source folder
	 * @return The checked folder path
	 */
	private String getCheckedAbsoluteSourceFolderPath(String absoluteSourceFolderPath)
	{
		//TODO check, if not exist, create
		File directory = new File(absoluteSourceFolderPath);
		if(directory.exists() == false) directory.mkdir();
		return absoluteSourceFolderPath;
	}
}
